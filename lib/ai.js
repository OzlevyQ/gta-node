...
async function generateWithGemini(prompt, model, options = {}) {
  try {
    ...
    const error = new Error('Gemini CLI not installed. Install: npm install -g @google/gemini-cli');
    error.cause = 'CLI not found in PATH';
    throw error;
  }

  // Modern gemini CLI: don't pass model - use default (works best)
  const args = [];

  // Add yolo flag to auto-approve
  args.push('--yolo');
  ...
  try {
    const { stdout } = await execa('gemini', args, { timeout: 30000 });

    // Extract the actual response text (remove any formatting)
    ...
    if (text.startsWith('{')) {
      try {
        ...
        return json.response || json.text || text;
      } catch {
        return text;
      }
    }
